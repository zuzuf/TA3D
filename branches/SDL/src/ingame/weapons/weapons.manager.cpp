...
namespace TA3D
{
    ...
    void WEAPON_MANAGER::load_tdf(char *data, const int size)
    {
        ...
        for(int i = 0 ; parser.exists( format("gadget%d", i) ) ; i++)
        {
            ...
            if (index >= 0)
            {
                weapon[index].damage = parser.pullAsInt( key + "damage.default" );
                #warning unit specific damage parser not implemented
                weapon[index].name = parser.pullAsString( key + "name" );
                weapon[index].weapon_id = parser.pullAsInt( key + "id" );
                weapon[index].rendertype = parser.pullAsInt( key + "rendertype" );
                weapon[index].ballistic = parser.pullAsBool( key + "ballistic" );
                weapon[index].turret = parser.pullAsBool( key + "turret" );
                weapon[index].noautorange = parser.pullAsBool( key + "noautorange" );
                weapon[index].range = parser.pullAsInt( key + "range" );
                weapon[index].time_to_range *= weapon[index].range;
                weapon[index].reloadtime = parser.pullAsFloat( key + "reloadtime" );
                weapon[index].weaponvelocity = parser.pullAsInt( key + "weaponvelocity" ) * 0.5f;
                weapon[index].time_to_range /= weapon[index].weaponvelocity;
                weapon[index].burst = parser.pullAsInt( key + "burst" );
                weapon[index].areaofeffect = parser.pullAsInt( key + "areaofeffect" );
                weapon[index].startsmoke = parser.pullAsBool( key + "startsmoke" );
                weapon[index].endsmoke = parser.pullAsBool( key + "endsmoke" );
                weapon[index].firestarter = parser.pullAsInt( key + "firestarter" );
                weapon[index].accuracy = parser.pullAsInt( key + "accuracy" );
                weapon[index].aimrate = parser.pullAsInt( key + "aimrate" );
                weapon[index].tolerance = parser.pullAsInt( key + "tolerance" );
                weapon[index].holdtime = parser.pullAsInt( key + "holdtime" );
                weapon[index].energypershot = parser.pullAsInt( key + "energypershot" );
                weapon[index].metalpershot = parser.pullAsInt( key + "metalpershot" );
                weapon[index].minbarrelangle = parser.pullAsInt( key + "minbarrelangle" );
                weapon[index].unitsonly = parser.pullAsBool( key + "unitsonly" );
                weapon[index].edgeeffectiveness = parser.pullAsFloat( key + "edgeeffectiveness" );
                weapon[index].lineofsight = parser.pullAsBool( key + "lineofsight" );
                {
                ...
                weapon[index].burstrate = parser.pullAsFloat( key + "burstrate" );
                weapon[index].duration = parser.pullAsFloat( key + "duration" );
                weapon[index].beamweapon = parser.pullAsBool( key + "beamweapon" );
                weapon[index].startvelocity = parser.pullAsInt( key + "startvelocity" ) * 0.5f;
                weapon[index].weapontimer = parser.pullAsFloat( key + "weapontimer" );
                weapon[index].weaponacceleration = parser.pullAsInt( key + "weaponacceleration" ) * 0.5f;
                weapon[index].turnrate = parser.pullAsInt( key + "turnrate" );
                weapon[index].model = model_manager.get_model( parser.pullAsString( key + "model" ) );
                weapon[index].smokedelay = parser.pullAsFloat( key + "smokedelay" );
                weapon[index].guidance = parser.pullAsInt( key + "guidance" );
                weapon[index].tracks = parser.pullAsBool( key + "tracks" );
                weapon[index].selfprop = parser.pullAsBool( key + "selfprop" );
                weapon[index].waterweapon = parser.pullAsBool( key + "waterweapon" );
                weapon[index].smoketrail = parser.pullAsBool( key + "smoketrail" );
                weapon[index].flighttime = parser.pullAsInt( key + "flighttime" );
                weapon[index].coverage = parser.pullAsInt( key + "coverage" ) * 0.5f;
                weapon[index].vlaunch = parser.pullAsBool( key + "vlaunch" );
                weapon[index].paralyzer = parser.pullAsBool( key + "paralyzer" );
                weapon[index].stockpile = parser.pullAsBool( key + "stockpile" );
                weapon[index].targetable = parser.pullAsBool( key + "targetable" );
                weapon[index].interceptor = parser.pullAsBool( key + "interceptor" );
                weapon[index].commandfire = parser.pullAsBool( key + "commandfire" );
                weapon[index].cruise = parser.pullAsBool( key + "cruise" );
                weapon[index].propeller = parser.pullAsBool( key + "propeller" );
                weapon[index].twophase = parser.pullAsBool( key + "twophase" );
                weapon[index].dropped = parser.pullAsBool( key + "dropped" );
                weapon[index].burnblow = parser.pullAsBool( key + "burnblow" );
                weapon[index].toairweapon = parser.pullAsBool( key + "toairweapon" );
                weapon[index].noexplode = parser.pullAsBool( key + "noexplode" );
                weapon[index].shakemagnitude = parser.pullAsInt( key + "shakemagnitude" );
                weapon[index].metal = parser.pullAsInt( key + "metal" );
                weapon[index].energy = parser.pullAsInt( key + "energy" );
                weapon[index].shakeduration = parser.pullAsFloat( key + "shakeduration" );
                weapon[index].waterexplosiongaf = parser.pullAsString( key + "waterexplosiongaf" );
                weapon[index].waterexplosionart = parser.pullAsString( key + "waterexplosionart" );
                weapon[index].lavaexplosiongaf = parser.pullAsString( key + "lavaexplosiongaf" );
                weapon[index].lavaexplosionart = parser.pullAsString( key + "lavaexplosionart" );
                weapon[index].explosiongaf = parser.pullAsString( key + "explosiongaf" );
                weapon[index].explosionart = parser.pullAsString( key + "explosionart" );
                weapon[index].soundtrigger = parser.pullAsString( key + "soundtrigger" );
                sound_manager->loadSound( weapon[index].soundtrigger , true );
                weapon[index].soundhit = parser.pullAsString( key + "soundhit" );
                sound_manager->loadSound( weapon[index].soundhit , true );
                weapon[index].soundstart = parser.pullAsString( key + "soundstart" );
                sound_manager->loadSound( weapon[index].soundstart , true );
                weapon[index].soundwater = parser.pullAsString( key + "soundwater" );
