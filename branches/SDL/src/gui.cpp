...
void button (float x,float y,float x2,float y2,const String &Title,bool Etat, SKIN *skin )
{
    if( skin && skin->button_img[ Etat ].tex )					// If we have gfx to skin the button then do it
    {
        ...
        {
            if (Etat)
                gfx->print(gui_font,(int)((x+x2)*0.5f-(gui_font->length(Title)*0.5f)+1),(int)((y+y2-(int)(gui_font->height()))*0.5f+1),0.0f, White,Title);
            else
                gfx->print(gui_font,(int)((x+x2)*0.5f-(gui_font->length(Title)*0.5f)),(int)(y+y2-(int)(gui_font->height()))*0.5f,0.0f,White,Title);
        }
    }
    else
    {
        gfx->rectfill(x,y,x2,y2,MGray);
        gfx->rect(x,y,x2,y2,Black);
        gfx->rect(x+1,y+1,x2-1,y2-1,DGray);
        if(!Etat) {
            gfx->line(x,y,x2,y,White);
            gfx->line(x,y,x,y2,White);
            gfx->line(x+1,y+1,x2-1,y+1,LGray);
            gfx->line(x+1,y+1,x+1,y2-1,LGray);
            if( !Title.empty() )
                gfx->print(gui_font,(int)((x+x2)*0.5f-(gui_font->length(Title)*0.5f)),(int)(y+y2-(int)(gui_font->height()))*0.5f,0.0f,White,Title);
        }
        else {
            gfx->line(x2,y2,x2,y,White);
            gfx->line(x2,y2,x,y2,White);
            gfx->line(x2-1,y2-1,x2-1,y+1,LGray);
            gfx->line(x2-1,y2-1,x+1,y2-1,LGray);
            if( !Title.empty() )
                gfx->print(gui_font,(int)((x+x2)*0.5f-(gui_font->length(Title)*0.5f)+1),(int)((y+y2-(int)(gui_font->height()))*0.5f+1),0.0f,White,Title);
        }
    }
...
void ListBox(float x1,float y1, float x2, float y2,const String::Vector &Entry,int Index, int Scroll, SKIN *skin, uint32 flags )
{
    if (skin && skin->text_background.tex)
    {
        ...
        for( i = 0 ; i < Entry.size() ; i++ )
        {
            ...
            gfx->print(gui_font,x1+skin->text_background.x1,y1+skin->text_background.y1+gui_font->height()*i,0.0f,White,str);
        }
        ...
        ScrollBar(	x2 + skin->text_background.x2 - skin->scroll[ 0 ].sw, y1 + skin->text_background.y1,
                    ...
                    TotalScroll ? ((float)Scroll) / TotalScroll : 0.0f, true, skin );
    }
    else
    {
        if (!(flags & FLAG_NO_BORDER))
        {
            gfx->rectfill(x1,y1,x2,y2,White);
            gfx->rect(x1,y1,x2,y2,DGray);
            gfx->rect(x1+1,y1+1,x2-1,y2-1,Black);
            gfx->line(x1,y2,x2,y2,LGray);
            gfx->line(x2,y1,x2,y2,LGray);
            gfx->line(x1+1,y2-1,x2-1,y2-1,White);
            gfx->line(x2-1,y1+1,x2-1,y2-1,White);
        }
        ...
        for(i=0;i<Entry.size();i++)
        {
            ...
            if (e == Index)
                gfx->rectfill( x1+4, y1+4+gui_font->height()*i, x2-4, y1+4+gui_font->height()*(i+1), Blue );
            gfx->print(gui_font,x1+4,y1+4+gui_font->height()*i,0.0f,White,Entry[e]);
        }
    }
}
...
int draw_text_adjust( float x1, float y1, float x2, float y2, String msg, int pos, bool mission_mode )
{
    String current = "";
    String current_word = "";
    ...
    else
    {
        for( int e = pos ; e < Entry.size() ; e++ )
            if( y1 + gui_font->height() * (e + 1 - pos) <= y2 )
                gfx->print( gui_font, x1, y1 + gui_font->height() * (e - pos), 0.0f, White, Entry[e] );
    }
...
void PopupMenu( float x1, float y1, const String &msg, SKIN *skin )
{
    ...
    if (skin && skin->menu_background.tex)
    {
        ...
        for( int e = 0 ; e < Entry.size() ; e++ )
            gfx->print(gui_font,x1+skin->menu_background.x1,y1+skin->menu_background.y1+gui_font->height()*e,0.0f,White,Entry[e]);
    }
    else
    {
        x2 += 8;
        ...
        gfx->rectfill(x1,y1,x2,y2,MGray);
        gfx->rect(x1,y1,x2,y2,Black);
        gfx->rect(x1+1,y1+1,x2-1,y2-1,DGray);
        gfx->line(x1,y1,x1,y2,White);
        gfx->line(x1,y1,x2,y1,White);
        gfx->line(x1+1,y1+1,x1+1,y2-1,LGray);
        gfx->line(x1+1,y1+1,x2-1,y1+1,LGray);

        for( int e = 0 ; e < Entry.size() ; e++ )
            gfx->print(gui_font,x1+4,y1+4+gui_font->height()*e,0.0f,White,Entry[e]);
    }
    Entry.clear();
}
...
void FloatMenu( float x, float y, const String::Vector &Entry, int Index, int StartEntry, SKIN *skin )
{
    if (skin && skin->menu_background.tex)
    {
        gfx->set_color( 0xFFFFFFFF );
        ...
        for (i = 0; i < Entry.size() - StartEntry; ++i)
        {
            ...
            if( e == Index )
            {
                if( skin->selection_gfx.tex )
                    ...
                    gfx->rectfill( x+skin->menu_background.x1,y+skin->menu_background.y1+gui_font->height()*i,x + width + skin->menu_background.x2,y+skin->menu_background.y1+gui_font->height()*(i+1), Blue );
            }
            gfx->print(gui_font,x+skin->menu_background.x1,y+skin->menu_background.y1+gui_font->height()*i,0.0f,White,Entry[e]);
        }
        gfx->unset_alpha_blending();
    }
    else
    {
        gfx->rectfill(x,y,x+168,y+8+gui_font->height()*(Entry.size() - StartEntry),MGray);
        gfx->rect(x,y,x+168,y+8+gui_font->height()*(Entry.size() - StartEntry),Black);
        gfx->rect(x+1,y+1,x+167,y+7+gui_font->height()*(Entry.size() - StartEntry),DGray);
        gfx->line(x,y,x,y+8+gui_font->height()*(Entry.size() - StartEntry),White);
        gfx->line(x,y,x+168,y,White);
        gfx->line(x+1,y+1,x+1,y+7+gui_font->height()*(Entry.size() - StartEntry),LGray);
        gfx->line(x+1,y+1,x+167,y+1,LGray);
        ...
        for( i=0 ; i<Entry.size() - StartEntry ; i++ )
        {
            int e = i + StartEntry;
            if( e == Index )
                gfx->rectfill( x+4,y+4+gui_font->height()*i,x+164,y+4+gui_font->height()*(i+1), Blue );
            gfx->print(gui_font,x+4,y+4+gui_font->height()*i,0.0f,White,Entry[e]);
        }
    }
}
...
void OptionButton(float x,float y,const String &Title,bool Etat, SKIN *skin)
{
    if (skin && skin->option[0].tex && skin->option[1].tex)
    {
        ...
        gfx->print(gui_font, x + skin->option[ Etat ? 1 : 0 ].sw + 4.0f, y + ( skin->option[ Etat ? 1 : 0 ].sh - gui_font->height() ) * 0.5f,0.0f,White,Title);
    }
    else
    {
        int x1,y1;

        gfx->circlefill(x+6,y+6,6,White);
        gfx->circle(x+6,y+6,6,MGray);
        gfx->circle(x+6,y+6,5,DGray);

        // Effet d'ombrage
        for(y1=(int)y ; y1<=y+12 ; y1++)
            for(x1=(int)x ; x1<=x+12 ; x1++)
                if((x1-x-6)*(x1-x-6)+(y1-y-6)*(y1-y-6)<49)
                    if(gfx->getpixel(x1,y1)==DGray&&x1-x+y1-y-6>6)
                        gfx->putpixel(x1,y1,LGray);

        if(Etat)
        {
            gfx->putpixel(x+6,y+6,DRed);
            gfx->circle(x+6,y+6,1,DRed);
            gfx->circle(x+6,y+6,2,DRed);
            gfx->circle(x+6,y+6,3,DRed);
            gfx->circlefill(x+6,y+6,2,DRed);
            gfx->putpixel(x+6,y+6,Red);
            gfx->circle(x+6,y+6,1,Red);
            gfx->circle(x+6,y+6,2,Red);
            gfx->circlefill(x+6,y+6,2,Red);
        }

        gfx->print(gui_font,x+16,y+3,0.0f,White,Title);
    }
}
...
void OptionCase(float x,float y,const String &Title,bool Etat, SKIN *skin )
{
    if (skin && skin->checkbox[0].tex && skin->checkbox[1].tex)
    {
        ...
        gfx->print(gui_font, x + skin->checkbox[ Etat ? 1 : 0 ].sw + 4.0f, y + ( skin->checkbox[ Etat ? 1 : 0 ].sh - gui_font->height() ) * 0.5f,0.0f,White,Title);
    }
    else
    {
        gfx->rectfill(x,y,x+12,y+12,White);
        gfx->rect(x,y,x+12,y+12,DGray);
        gfx->rect(x+1,y+1,x+11,y+11,Black);
        gfx->line(x,y+12,x+12,y+12,LGray);
        gfx->line(x+12,y,x+12,y+12,LGray);
        gfx->line(x+1,y+11,x+11,y+11,White);
        gfx->line(x+11,y+1,x+11,y+11,White);

        if (Etat)
        {
            gfx->line(x+3,y+6,x+5,y+9,DGray);
            gfx->line(x+5,y+9,x+8,y+4,DGray);
            gfx->line(x+3,y+5,x+5,y+8,DGray);
            gfx->line(x+5,y+8,x+8,y+3,DGray);
        }
...
//    set_uformat( U_ASCII );
    ...
    if (skin && skin->text_background.tex)
    {
        ...
        gfx->print(gui_font,x1+skin->text_background.x1,y1+skin->text_background.y1+skin->text_y_offset,0.0f,White,strtoprint);
		if (blink) gfx->print(gui_font,x1+skin->text_background.x1+gui_font->length( strtoprint ),y1+skin->text_background.y1+skin->text_y_offset,0.0f,White,"_");
    }
    else
    {
        gfx->rectfill(x1,y1,x2,y2,White);
...
/*---------------------------------------------------------------------------\
  |                              Draw a scroll bar                             |
  \---------------------------------------------------------------------------*/
void ScrollBar( float x1, float y1, float x2, float y2, float Value, bool vertical, SKIN *skin)
{
    if (skin)
    {
        gfx->set_color( 0xFFFFFFFF );
...
void ProgressBar(float x1,float y1,float x2,float y2,int Value, SKIN *skin )
{
    ...
    {
        gfx->rectfill(x1,y1,x2,y2,White);
        gfx->rectfill(x1+2,y1,x1+1+(x2-x1-3)*Value/100,y2,Blue);
        gfx->rect(x1,y1,x2,y2,DGray);
        gfx->rect(x1+1,y1+1,x2-1,y2-1,Black);
        gfx->line(x1,y2,x2,y2,LGray);
        gfx->line(x2,y1,x2,y2,LGray);
