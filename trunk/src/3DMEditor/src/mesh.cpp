...
void Mesh::computeAmbientOcclusion(int w, int h, Mesh *base, int precision, MeshTree *baseTree, QImage *img)     // For the Mesh and each submesh, create a texture representing global occlusion generated by the mesh itself
{
    if (base == NULL)
        base = this;
    ...
    {
        deleteTree = true;
        singleMesh = base->toSingleMesh();
        baseTree = new MeshTree;
    ...
    for(int i = 0 ; i < precision ; i++)
    {
        ...
        float beta = asinf((abs(qrand()) % 10000) * 0.0001f);
        dirs.push_back( Vec(cosf(beta) * cosf(alpha), cosf(beta) * sinf(alpha), sinf(beta)) );
    }
    ...
    if (!img)
    {
        ...
        img->fill(0);            // By default, occlusion is total
    }
    int limit(0);
    int step(1);
    switch(type)
    ...
    {
        if (child)
            child->computeAmbientOcclusion(w, h, base, precision, baseTree, img);
        if (next)
            next->computeAmbientOcclusion(w, h, base, precision, baseTree, img);
    }
