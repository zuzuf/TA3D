...
void Mesh::computeAmbientOcclusion(int w, int h, Mesh *base)     // For the Mesh and each submesh, create a texture representing global occlusion generated by the mesh itself
{
    if (base == NULL)
        base = this;
    ...
    QImage ID(w, h, QImage::Format_RGB888);
    ...
    for(int i = 0 ; i < limit ; i += step)          // Render triangles using their (ID + 1) as color
    {
        ...
        if (a.y != b.y)
            for(int y = a.y ; y < b.y ; y++)
            {
                ...
                for(int x = x0 ; x <= x1 ; x++)
                {
                    Vec D = P0 + ((float)(x - x0)) / (x1 - x0) * (P1 - P0);
                    ...
                    for(int i = 0 ; i < 25 ; i++)
                    {
                        float alpha = (abs(qrand()) % 360) * Deg2Rad;
                        float beta = (abs(qrand()) % 90) * Deg2Rad;
                        Vec Dir = sinf(beta) * N + cosf(beta) * (cosf(alpha) * I + sinf(alpha) * J);
                        if (base->hit(D, Dir, K) != -1)
                            n++;
                    }
                    n = 255 - n * 10;
