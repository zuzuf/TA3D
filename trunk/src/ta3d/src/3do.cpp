	...
	bool OBJECT::draw(float t,ANIMATION_DATA *data_s,bool sel_primitive,bool alset,bool notex,int side,bool chg_col,bool exploding_parts)
	{
		...
		void MODEL::load_asc(const String& filename,float size)		// Charge un fichier au format *.ASC
		{
			...
			do
			{
				...
				String s_chaine(chaine, strlen(chaine));
				s_chaine.toUpper();
				if (s_chaine.startsWith("VERTEX"))
				{
					if (s_chaine.startsWith("VERTEX LIST"))
					{
				...
				{
					if (s_chaine.startsWith("FACE"))
					{
						if (s_chaine.startsWith("FACE LIST"))
						{
					...
					{
						if (s_chaine.startsWith("NAMED OBJECT"))
						{
							char *_p = (chaine + 13);
							while (_p[0])
							{
							...
							StructName[NbStruct].assign(chaine + 13);
							StructName[NbStruct].trim(" \"");
							decalage = nbp;
