...
//----- Load game information --------------------------------------------------------------
	...
	foreach( game_data->player_names, i )		*i = readstring( tmp, 1024, file );
	foreach( game_data->player_sides, i )		*i = readstring( tmp, 1024, file );
	foreach( game_data->player_control, i )		*i = fgetc( file );
	foreach( game_data->player_network_id, i )	LOAD( *i );
...
//----- Load unit information --------------------------------------------------------------
	...
	for( int e = 0 ; e < units.nb_unit ; e++ ) {
		...
		foreach( *units.unit[i].s_var, f )	LOAD( *f );

		LOAD( g );
		units.unit[i].script_val->resize(g);
		foreach( *units.unit[i].script_val, f )	LOAD( *f );
		...
		fread( units.unit[i].data.pos, sizeof( VECTOR ), units.unit[i].data.nb_piece, file );
		fread( units.unit[i].data.dir, sizeof( VECTOR ), units.unit[i].data.nb_piece, file );
