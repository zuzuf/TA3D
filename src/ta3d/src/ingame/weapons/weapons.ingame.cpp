...
namespace TA3D
{
	...
	void InGameWeapons::init(bool real)
	{
		...
		if (real)
		{
            QIODevice *file = VFS::Instance()->readFile("anims/fx.gaf");
			if (file)
			{
	...
	void InGameWeapons::proc(void*)
	{
		...
		thread_running = false;
		thread_ask_to_stop = false;
		LOG_INFO("Weapon engine: " << (float)(counter * 1000) / float(msectimer() - weapon_timer) << " ticks/sec");
	}
