...
namespace Menus
{
	...
	bool Animator::userInteraction()
	{
		...
		if (cursor_idx != nidx)
		{
			...
			result = true;
			pArea->caption("animator.pointed", cursor_idx >= 0 ? TA3D::VARS::obj_table[cursor_idx]->name : "");
		}
		if (cursor_idx >= 0 && mouse_b == 1)
		{
			switch(pointing_mode)
			{
				case ANIMATOR_MODE_SELECTION:
					sel_idx = cursor_idx;
					result = true;
					pArea->caption("animator.selected", TA3D::VARS::obj_table[sel_idx]->name);
					break;
	...
	void Animator::renderModel()
	{
		...
		for (float y = -100.f ; y <= 100.f ; y += 10.f)
		{
			if (y == 0.0f)
			{
