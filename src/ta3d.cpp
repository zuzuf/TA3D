...
int play(GameData *game_data)
{
    ...
    do
    {
        ...
        if(selected && ( !IsOnGUI || IsOnMinimap ) )
        {
            ...
            if(pointing>=0 && !rope_selection) 	// S'il y a quelque chose sous le curseur
            {
                ...
                if(mouse_b!=1 && omb3==1)
                    if(cursor_type==CURSOR_ATTACK)
                    {
                        VECTOR cursor_pos = cursor_on_map(&cam,map,IsOnMinimap);
                        for(uint16 e=0;e<units.index_list_size;e++)
                        {
                            uint32 commandfire = current_order == SIGNAL_ORDER_DGUN ? MISSION_FLAG_COMMAND_FIRE : 0;
                            i = units.idx_list[e];
        ...
        if( IsOnGUI && !IsOnMinimap )	cursor_type=CURSOR_DEFAULT;
        ...
        if(g_useProgram && g_useFBO && lp_CONFIG->water_quality>=2 && map->water && !map->ota_data.lavaworld && !reflection_drawn_last_time) {		// Dessine les reflets sur l'eau
            ...
            if(cam.rpos.y<=gfx->low_def_limit && lp_CONFIG->water_quality==4)
            {
                ...
                weapons.draw(&refcam,map);			// Dessine les objets produits par les armes / draw weapons
        ...
        units.draw(&cam,map,true,false,true,lp_CONFIG->height_line);			// Dessine les unités sous l'eau / Draw units which are under water
        ...
        if(map->water)
        {
            ...
            else
            {
                ...
                glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,water_FBO);					// First pass of water rendering, store reflection vector
        ...
        if(sel>=0 && mouse_b==2 && omb2!=2) {
            ...
            int nb=1;
            if(TA3D_SHIFT_PRESSED) nb=5;
            if(cur)	{	old=&(cur->next);	cur=cur->next; }
            if(cur)
                while(cur) {
        ...
        if(mouse_x>=32 && mouse_x<=95 && mouse_y>=SCREEN_H-64 && omb2==0) {
            ...
            else if(mouse_b==2 && !freecam) {				// Reset default view
        ...
        else if( internal_idx && cur_sel_index >= 0 )
